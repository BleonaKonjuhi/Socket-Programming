...
def handle_request(data, conn):
    try:
        ...
        response = "Gjate thirrjes ka ndodhur nje gabim,provoni perseri"
        ShtypTeDhenat("ERROR")
        conn.sendall(str.encode(response))
...
s.bind((host,port))
# pasiqe te krijohet socketi i serverit te TCP protokolli
#  presim per nje klient te lidhet me server:
s.listen(5)
